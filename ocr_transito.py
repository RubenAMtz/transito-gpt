...
og_text = extract_text("docs/leyseguridadpublica.pdf")
name = 'leyseguridadpublicav1' # change this to the name of the csv and json files you want to save
...
# ARTÍCULO \d+ BIS A.-, ARTÍCULO \d+ BIS B.-, etc.
# ARTÍCULO \d+ TER.-
# There is a typo in the document, it says 'ARÍCULO 163.-' instead of 'ARTÍCULO 163.-'
# find all the matches of 'ARTICULO \d+.-' and 'ARTICULO \*\d+.-'
matches = re.finditer(r'ARTICULO \d+.-| ARTICULO \*\d+.- | ARTICULO \d+o.- | ARTICULO \*\d+o.- | ARTÍCULO \d+.- | ARTÍCULO \d+º.- | ARTÍCULO \*\d+.- | ARTÍCULO \d+ BIS.- | ARTÍCULO \d+ BIS [A-Z].- | ARTÍCULO \d+ TER.- | ARÍCULO \d+.-', og_text)
...
print("Number of articulos:", len(articulos))

# initialize pdf
pdf = {}
# loop through articulos and print the number of tokens
for i, (articulo, group) in enumerate(zip(articulos, groups)):
    ...
    if lenght > split_len:
        ...
        for j in range(0, lenght, stride):
            if j+split_len <= lenght:
                ...
                pdf[f'{group}'][f'parte_{j//stride+1:02}'] = gpt_encoding.decode(encoded_article[j:j+split_len])
            else:
                # pdf[f'articulo {i+1}'][f'parte_{j//stride+1:02}'] = gpt_encoding.decode(encoded_article[j:])
                pdf[f'{group}'][f'parte_{j//stride+1:02}'] = gpt_encoding.decode(encoded_article[j:])
    else:
        # pdf[f'articulo {i+1}']['parte_01'] = articulo
        pdf[f'{group}']['parte_01'] = articulo
